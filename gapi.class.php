...
class gapi
{
  ...
  private function curlRequest($url, $get_variables=null, $post_variables=null, $headers=null)
  {
    ...
    echo $get_variables['dimensions'];

    $ch = curl_init();
    
     if($get_variables['dimensions'] == 'ga:all')
    {

    $new_array = array('ids' => 'ga:59307293',
    //'metrics' => 'ga:pageviews,ga:visits,ga:visitors,ga:percentNewVisits,ga:entranceBounceRate,ga:pageviewsPerVisit,ga:avgTimeOnPage',
    'metrics' => 'ga:visits,ga:avgTimeOnSite',
    'start-date' => '2012-07-31',
    'end-date' => '2012-07-31',
    'max-results' => '100');

      $get_variables = '?' . str_replace('&amp;','&',urldecode(http_build_query($new_array)));
    } else {
      //Is not make all report
